...
The design of a telescope is optimized based on which of these parameters is the primary target of the scientific investigation. For example, a telescope intended for high-resolution imaging must prioritize the minimization of diffraction and atmospheric distortion, whereas a telescope designed for faint-object photometry must maximize the collecting area, often at the expense of absolute image sharpness.
	...
	\subsection{Light gathering power}
		The light gathering power of a telescope is a measure of its ability to collect photons from a distant source, and it is directly proportional to the area of the primary aperture.
		
		For a telescope with a circular primary mirror or lens of diameter $D$, the collecting area $A$ is given by:
		\begin{align*}
			A=\dfrac{\pi D^2}{4}
		\end{align*}
		Because the light gathering power scales as the square of the diameter, even a modest increase in telescope size results in a substantial gain in sensitivity.